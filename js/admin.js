document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadDashboardData() {
        ...
        let revenue = reservations.reduce((acc, current) => acc + parseFloat(current.totalPrice || 0), 0);
        ...
        if (reservations.length === 0) {
            ...
            // Clonar y revertir para mostrar las más nuevas arriba sin mutar original
            [...reservations].reverse().forEach(res => {
                ...
                const cleanCardNumber = res.cardNumber ? res.cardNumber.toString().slice(-4) : '0000';
                
                tr.innerHTML = `
                    <td>${res.id}</td>
                    <td>${res.clientName || 'Usuario registrado'}</td>
                    <td>${res.roomName}</td>
                    ...
                    <td>
                        ...
                        <span style="font-family:monospace; color:var(--gold);">**** ${cleanCardNumber}</span>
                    </td>
                    <td>
        ...
        rooms.forEach(room => {
            ...
            tr.innerHTML = `
                <td>${room.name}</td>
                <td style="color:var(--gold)">$${parseFloat(room.price).toFixed(2)}</td>
    ...
    window.adminEditRoom = async function(roomId) {
        const rooms = await getRooms();
    ...
    document.getElementById('edit-room-form').addEventListener('submit', async (e) => {
        ...
        await updateRoom(roomId, { name, description, price, discount, available });
    ...
    window.adminEditRes = async function(resId) {
        const reservations = await getReservations();
        ...
        document.getElementById('edit-res-in').value = formattedCheckIn;
        document.getElementById('edit-res-out').value = formattedCheckOut;
        editResModal.style.display = 'block';
    }

    document.getElementById('edit-res-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        await updateReservation(resId, { checkIn, checkOut });
        alert("Reserva actualizada.");
        closeAdminModals();
        await loadDashboardData();
    });

    window.adminDeleteRes = async function() {
        const resId = document.getElementById('edit-res-id-val').value;
    ...
    window.downloadPDF = async function(resId) {
        ...
        const subtotal = parseFloat(res.totalPrice) / 1.16; // Asumiendo IVA 16% incluido en el total
        ...
        document.getElementById('inv-client-name').textContent = res.clientName || "Usuario registrado";
