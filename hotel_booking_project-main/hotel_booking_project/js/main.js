document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('register-form').addEventListener('submit', (e) => {
        ...
        const result = registerUser(fname, lname, email, pwd);
        if (result.success) {
    ...
    function openProfileModal() {
        ...
        tbody.innerHTML = '';
        const userRes = getUserReservations(user.email);

        if (userRes.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="padding: 10px;">No tienes reservas aún.</td></tr>';
        } else {
            userRes.forEach(res => {
                tbody.innerHTML += `
                    <tr style="border-bottom: 1px solid #e2e8f0;">
                        <td style="padding: 10px;">${res.id}</td>
                        <td style="padding: 10px;">${res.roomName}</td>
                        <td style="padding: 10px;">${res.checkIn}</td>
                        <td style="padding: 10px;">${res.checkOut}</td>
                        <td style="padding: 10px;">$${res.totalPrice.toFixed(2)}</td>
...
document.getElementById('save-card-form').addEventListener('submit', (e) => {
    e.preventDefault();
    const user = getCurrentUser();
    if (!user) return;
    ...
    if (saveUserCard(user.email, cardInfo)) {
...
window.downloadPDF = function(resId) {
    ...
    const reservations = getUserReservations(user.email);
    ...
    const subtotal = res.totalPrice / 1.16; // Asumiendo IVA 16% incluido en el total para que coincida con el diseno
    ...
    document.getElementById('inv-guests').textContent = "Según habitación"; // Se podría guardar al reservar
    ...
    // Hacemos que el template sea visible solo para html2pdf (está oculto por defecto con display:none en su padre)
